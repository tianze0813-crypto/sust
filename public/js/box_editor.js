...
function BoxEditorManager(parentUi, viewManager, objectTrackView, 
    ...
    {
        // Snapshot once per world so batch finalize can be undone with Ctrl+Z.
        let snappedWorlds = new Set();
        this.getSelectedEditors().forEach(e=>{
            if (e.box && e.box.world && e.box.world.annotation){
                let world = e.box.world;
                if (!snappedWorlds.has(world)){
                    world.annotation.undoManager.takeSnapshot(world.annotation);
                    snappedWorlds.add(world);
                }
            }
        });
    ...
    this.undo = function(){
        ...
        let candidates = selectedEditors.length > 0 ? selectedEditors : this.activeEditorList();
        let touched = false;

        candidates.forEach(e=>{
            let ann = e && e.target && e.target.world && e.target.world.annotation;
            if (ann && ann.undo()){
                touched = true;
            }
        });
    ...
    this.finalizeUpToThisFrame = function(){
        if (!this.firingBoxEditor){
            return;
        }

        let targetIndex = this.firingBoxEditor.index;
        let affectedEditors = this.activeEditorList().filter(e=> e.index <= targetIndex && e.box);

        if (affectedEditors.length === 0){
            return;
        }

        this.activeEditorList().forEach(e=>e.setSelected(false));
        affectedEditors.forEach(e=>e.setSelected(true, "finalize-up-to-frame"));
        this.finalizeSelectedBoxes();

        window.editor.infoBox.show("Info", `Finalized ${affectedEditors.length} frame(s) up to ${this.firingBoxEditor.target.world.frameInfo.frame}.`);
    };
    ...
    this.handleContextMenuEvent = function(event)
    {
        ...
        case 'cm-finalize':
            this.finalizeSelectedBoxes();
            break;
        case 'cm-finalize-up-to-frame':
            this.finalizeUpToThisFrame();
            break;
        ...
        case 'cm-follow-static-objects':
            {
                ...
                editor.autoAdjust.followStaticObjects(b).then(()=>{
                    this.globalHeader.updateModifiedStatus();
                    ...
                    console.error("batch follow static objects failed", error);
                });
    ...
    this.reset = function(){
        ...
        this.activeIndex = 0;
        this.autoAnnotateSession.running = false;
        this.setAutoAnnotateToolState();
    };
    ...
    this.autoAnnotateSession = {
        running: false,
    };

    this.setAutoAnnotateToolState = function(){
        let autoBtn = this.toolbox.querySelector("#auto-annotate");
        let autoTranslateBtn = this.toolbox.querySelector("#auto-annotate-translate-only");

        if (this.autoAnnotateSession.running){
            autoBtn.style.opacity = "0.5";
            autoTranslateBtn.style.opacity = "0.5";
        } else {
            autoBtn.style.opacity = "1";
            autoTranslateBtn.style.opacity = "1";
        }
    };

    this.ensureBatchEditorIdle = function(){
        if (!this.autoAnnotateSession.running){
            return true;
        }

        window.editor.infoBox.show("Info", "Auto annotation is running.");
        return false;
    };
    ...
    this.autoAnnotate = async function(applyIndList, dontRotate){
        if (this.autoAnnotateSession.running){
            window.editor.infoBox.show("Info", "Auto annotation is already running.");
            return;
        }
        ...
        this.autoAnnotateSession.running = true;
        this.setAutoAnnotateToolState();
    ...
    this.object_track_id_changed = function(event){
        ...
        this.activeEditorList().forEach(e=>{
            if (e.box){
                if (e.box.world && e.box.world.annotation) {
                    e.box.world.annotation.undoManager.takeSnapshot(e.box.world.annotation);
                }
