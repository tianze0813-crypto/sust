...
PCDLoader.prototype = {
	...
	parseBin: function(data, url){
		...
		var color = [];
		var intensity = [];
		// Default to xyzi. Fall back to xyz for compact bin files.
		var fields = 4;
		if ( data.byteLength % 16 !== 0 && data.byteLength % 12 === 0 ) {
			fields = 3;
		}
		if ( data.byteLength % ( 4 * fields ) !== 0 ) {
			throw new Error( 'Unsupported bin point cloud byte length: ' + data.byteLength );
		}
		var rowSize = 4 * fields;

		for ( var row = 0; row < data.byteLength/rowSize; row += 1 ) {
			var offset = row * rowSize;
			position.push( dataview.getFloat32( offset + 0, this.littleEndian ) );
			position.push( dataview.getFloat32( offset + 4, this.littleEndian ) );
			position.push( dataview.getFloat32( offset + 8, this.littleEndian ) );
			if ( fields >= 4 ) {
				intensity.push(dataview.getFloat32( offset + 12, this.littleEndian ) )
			}
		}
