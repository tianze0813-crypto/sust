...
import {ImageContextManager} from "./image.js?v=4";
...
function Editor(editorUi, wrapperUi, editorCfg, data, name="editor"){
    ...
    this.init = function(editorUi) {
        ...
        this.initProjectBoxToImageButton(editorUi.querySelector("#project-box-to-image-button"));
    ...
    this.initProjectBoxToImageButton = function(button){
        if (!button)
            return;

        let updateState = ()=>{
            button.className = this.editorCfg.projectBoxToImage ? "ui-button ui-button-active" : "ui-button";
        };

        button.onclick = (event)=>{
            let enabled = !this.editorCfg.projectBoxToImage;
            pointsGlobalConfig.setItem("projectBoxToImage", enabled);
            this.editorCfg.projectBoxToImage = enabled;
            updateState();
