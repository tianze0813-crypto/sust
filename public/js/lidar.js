...
function Lidar(sceneMeta, world, frameInfo){
    ...
    this.preload=function(on_preload_finished){
        ...
        loader.load( this.frameInfo.get_pcd_path(), 
            ...
            function(error){
                console.error(
                    "load point cloud failed",
                    _self.frameInfo.get_pcd_path(),
                    error
                );
                _self._afterPreload();
            },
    ...
    // Keep the old misspelled API for plugins and older code.
    this.get_all_pionts=this.get_all_points;
    ...
    this.select_points_by_view_rect=function(x,y,w,h, camera){
        ...
        if (!points || !points.geometry) {
            console.warn("select rect ignored: point cloud is not loaded");
            return [];
        }
        var pos_array = points.geometry.getAttribute("position").array;
