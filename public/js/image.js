...
class ImageContext extends MovableView{
    ...
    get_box_image_label(box){
        let id = box.obj_track_id;
    ...
    get_box_image_label_position(imgfinal){
        let xs = [];
        let ys = [];
        for (let i = 0; i < imgfinal.length / 2; i++){
            let x = imgfinal[i*2];
        ...
        let x = Math.max(8, Math.min(2040, minX));
        let y = (minY > 36) ? minY - 10 : maxY + 30;
        y = Math.max(30, Math.min(1528, y));
        return {x: x, y: y};
    }

    set_box_id_text_attr(text, box, imgfinal){
        let pos = this.get_box_image_label_position(imgfinal);
        if (!pos){
            text.textContent = "";
            return;
        }

        text.textContent = this.get_box_image_label(box);
        text.setAttribute("x", Math.round(pos.x));
        text.setAttribute("y", Math.round(pos.y));
    }
    ...
    box_to_svg(box, box_corners, trans_ratio, selected){
        ...
        */
        ...
        var id_text = document.createElementNS("http://www.w3.org/2000/svg", "text");
        svg.appendChild(id_text);
        id_text.setAttribute("class", "image-box-id");
        this.set_box_id_text_attr(id_text, box, imgfinal);
    ...
    boxes_manager = {
        ...
        update_box: (box)=>{
            ...
            if (imgfinal){
                ...
                let id_text = b.querySelector(".image-box-id");
                if (id_text){
                    this.set_box_id_text_attr(id_text, box, imgfinal);
                }
            }
...
function get_calib_model_type(calib){
    if (calib && calib.model_type){
        return String(calib.model_type).toUpperCase();
    }
    return "PINHOLE";
}

function get_calib_distortion(calib){
    if (!calib){
        return [];
    }
    return calib.distortion || calib.D || [];
}

function project_point_camera_model(calib, x, y, z){
    let K = calib.intrinsic || [];
    let D = get_calib_distortion(calib);
    let model = get_calib_model_type(calib);

    if (model.indexOf("PINHOLE") === 0){
        if (Math.abs(z) <= 1e-9){
            return null;
        }
        ...
        let k1 = d[0], k2 = d[1], p1 = d[2], p2 = d[3], k3 = d[4], k4 = d[5], k5 = d[6], k6 = d[7];
        let r2 = xn * xn + yn * yn;
        let r4 = r2 * r2;
        let r6 = r4 * r2;
        let radialNum = 1.0 + k1 * r2 + k2 * r4 + k3 * r6;
        let radialDen = 1.0 + k4 * r2 + k5 * r4 + k6 * r6;
        if (Math.abs(radialDen) <= 1e-9){
            return null;
        }
        let radial = radialNum / radialDen;
    ...
    for (let i = 0; i < Math.min(4, D.length); i++){
        d[i] = parseFloat(D[i]);
    }
    ...
    let scale = (r > 1e-9) ? (theta_d / r) : 1.0;
    let xd = x * scale;
    let yd = y * scale;
...
function is_projectable_point(calib, x, y, z){
    let model = get_calib_model_type(calib);
    if (model.indexOf("PINHOLE") === 0){
        return z > 1e-6;
    }
...
function points3d_homo_to_image2d(points3d, calib, accept_partial=false,save_map, img_dx, img_dy){
    ...
    for (var i = 0; i < imgpos3.length/3; i++){
        ...
        if (!is_projectable_point(calib, x, y, z)){
            if (!accept_partial){
                return null;
            }
            continue;
        ...
        let uv = project_point_camera_model(calib, x, y, z);
        if (!uv || !isFinite(uv[0]) || !isFinite(uv[1])){
            if (!accept_partial){
                return null;
            }
            continue;
        ...
        let px = Math.round(uv[0]);
        let py = Math.round(uv[1]);

        if (accept_partial){
            if (img_dx && img_dy && px > 0 && px < img_dx && py > 0 && py < img_dy){
                if (save_map){
                    save_map[img_dx * py + px] = [i, points3d[i*4+0], points3d[i*4+1], points3d[i*4+2]];
                }
                imgfinal.push(px);
                imgfinal.push(py);
            }
    ...
    if (!accept_partial && imgfinal.length !== imgpos3.length / 3 * 2){
        return null;
    }
...
function  choose_best_camera_for_point(scene_meta, center){
    ...
    for (var i in scene_meta.calib.camera){
        var calib = scene_meta.calib.camera[i];
        var imgpos = matmul(calib.extrinsic, [center.x,center.y,center.z,1], 4);
        var pos = vector4to3(imgpos);
        if (is_projectable_point(calib, pos[0], pos[1], pos[2])){
            proj_pos.push({calib: i, pos: pos});
        }
    }

    proj_pos.forEach(function(p){
        p.dist_to_center = p.pos[0]*p.pos[0] + p.pos[1]*p.pos[1];
    });

    proj_pos.sort(function(x,y){
        return x.dist_to_center - y.dist_to_center;
    });

    //console.log(valid_proj_pos);

    if (proj_pos.length>0){
        return proj_pos[0].calib;
    }
