...
import {Lidar} from "./lidar.js?v=2"
import {Annotation} from "./annotation.js?v=2"
...
function FrameInfo(data, sceneMeta, sceneName, frame){
    ...
    this.get_pcd_path = function(){
            if (this.sceneMeta.lidar_fusion && this.sceneMeta.lidar_fusion.enabled){
                return `/fused_lidar?scene=${encodeURIComponent(this.scene)}&frame=${encodeURIComponent(this.frame)}`;
            }
            if (this.sceneMeta.frame_files && this.sceneMeta.frame_files[this.frame] && this.sceneMeta.frame_files[this.frame].lidar){
                let lidarFile = this.sceneMeta.frame_files[this.frame].lidar._default;
                if (lidarFile){
                    return 'data/'+ this.scene + "/lidar/" + lidarFile;
                }
            }
            return 'data/'+ this.scene + "/lidar/" + this.frame + this.sceneMeta.lidar_ext;
        };
    this.get_camera_path = function(name){
        let cameraDir = this.sceneMeta.camera_dir || "camera";
        if (this.sceneMeta.frame_files && this.sceneMeta.frame_files[this.frame] && this.sceneMeta.frame_files[this.frame].camera){
            let cameraFile = this.sceneMeta.frame_files[this.frame].camera[name];
            if (cameraFile){
                return `data/${this.scene}/${cameraDir}/${name}/${cameraFile}`;
            }
        }
        return `data/${this.scene}/${cameraDir}/${name}/${this.frame}${this.sceneMeta.camera_ext}`;
    };
...
function Images(sceneMeta, sceneName, frame, frameInfo){
    ...
    this.load = function(on_all_loaded, active_name){
        ...
        this.names.forEach(function(cam){
            ...
            _self.content[cam].onerror=function(){ 
                _self.loaded_flag[cam] = true;
                console.error("image load failed", cam, _self.content[cam].src);
                _self.on_image_loaded();
            };

            if (frameInfo){
                _self.content[cam].src = frameInfo.get_camera_path(cam);
            } else {
                let cameraDir = sceneMeta.camera_dir || "camera";
                _self.content[cam].src = 'data/'+sceneName+'/'+cameraDir+'/' + cam + '/'+ frame + sceneMeta.camera_ext;
            }
...
function World(data, sceneName, frame, coordinatesOffset, on_preload_finished){
    ...
    this.cameras = new Images(this.sceneMeta, sceneName, frame, this.frameInfo);
